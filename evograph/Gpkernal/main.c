...
//HJJ extern pthread_mutex_t random_mutex;

multipop *mpop;//HJJM make mpop a global variable
...
int main ( int argc, char **argv )
{
     ...
     /* Added by Sean.  Initialize the random mutex */

		//HJJ pthread_mutex_init(&random_mutex,NULL);
...
//	 printf(" after breed		level %d include %d subpop    \n", 1, (*mpop->pHF)[0]->subpop_list.size());
     ...
     /* Added By Sean */
//HJJ     pthread_mutex_destroy(&random_mutex);
...
//!read GP run options. use HFC? use SFS?
void read_gp_run_option()
{
	//HJJ read more options/switches  from input file
	//get the global data structure
	globaldata* g = get_globaldata();//get the lists

	g->bUseStructParamEvoAlternation=false;
	g->maxParamEvoGen=10;	//how many generations of parameter evolution after structure evolution
	g->maxStructEvoGen=1;	//how many generations of structure evolution after parameter evolution
	
	
	//Do we use StructParamEvoAlternation?
	char* paramString;
	paramString = get_parameter ( "StructParamEvolution" );
	if(paramString==NULL)
		g->bUseStructParamEvoAlternation=false;
	else{
		...
		if(i==0)
			...
			g->bUseStructParamEvoAlternation=true;
			printf(" Using StructParamEvoAlternation technique.....!\n");
		}
	}

	//Do we use probabilistic alternation or fixed generation alternation
	if(g->bUseStructParamEvoAlternation==true){
				paramString = get_parameter ( "StructParamEvolution.probabilistic" );
				if(paramString==NULL)
					g->bProbabilisticSPA=false;
				else{
					int i=atoi(paramString);
					if(i!=0){
						g->bProbabilisticSPA=true;
						printf(" Using probablistic StructParamEvoAlternation technique.....!\n");
					}
					else{
						g->bProbabilisticSPA=false;
					}
	...
	//The  probability of crossover with structure operation nodes
	if(g->bUseStructParamEvoAlternation==true){
			paramString = get_parameter ( "probStructEvoStage" );
			if(paramString==NULL)
				g->probStructEvoStage=0.2;
			else{
				double i=atof(paramString);
				if(i!=0){
					g->probStructEvoStage=i;
					printf(" probStructEvoStage= %f.....!\n", g->probStructEvoStage);
				}
				else
					g->probStructEvoStage=0.2;
			}
	...
	//Do we use HFC: Hierarchical Fair Competition model?
	paramString = get_parameter ( "multiple.HFCPM" );
	if(paramString==NULL)
		g->bUseHFC=false;
	else{
		int i=atoi(paramString);
	...
	//Do we use Adaptive HFC (threshold changes)
	if(g->bUseHFC==true){
		paramString = get_parameter ( "multiple.HFCPM.adaptive" );
		if(paramString==NULL)
			...
			if(i!=0){
				g->bAdaptiveHFC=true;
				printf(" Using HFC with adaptive admission fitness technique.....!\n");
			}
			else
				g->bAdaptiveHFC=false;
		}
	...
	//Do we use SFS: structure fitness sharing technique?
	paramString = get_parameter ( "bUseSFS" );
	if(paramString==NULL)
		g->bUseSFS=false;
	else{
		int i=atoi(paramString);
		if(i!=0){
			g->bUseSFS=true;
			printf(" Using Struct Fitness Sharing technique.....!\n");
		}
		else
			g->bUseSFS=false;
	}

	//Do we use writhe history GP tree
	paramString = get_parameter ( "saveHistroyData" );
	if(paramString==NULL)
		g->saveHistoryData=false;
	else{
		int i=atoi(paramString);
		if(i!=0){
			g->saveHistoryData=true;
			printf(" Save history gp trees.....!\n");
		}
		else
			g->saveHistoryData=false;
	}
