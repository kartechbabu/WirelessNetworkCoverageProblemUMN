 ...
 *	With some modification for SFS and HFC
 *	
...
{ { "crossover",      operator_crossover_init },
  ...
  { "mutation",       operator_mutate_init },
	{ "mutate_erc",	  operator_mutate_erc_init}, //HJJ_ME mutate constant ERC
  { NULL, NULL } };
          ...
          *namep = 0;
          ...
          if ( op->name )
	       ...
	       /* the specified operator is not in the table. */
               error ( E_FATAL_ERROR,"%s: \"%s\" is not a known operator.", pnamebuf, param );
...
//HJJ_ELITISM
//Test if the current subpop belongs to the highest active level of the whole populations
int IsHighestActiveLevel(int subpopID)
{
	...
	for( i=0;i<mpop->pHF->size();i++){  //for each level
			 for( j=0;j<(*mpop->pHF)[i]->subpop_list.size();j++){//for each subpop of this level
				 int subpop = (*mpop->pHF)[i]->subpop_list[j];
				 if(mpop->subpopActiveFlag[subpop]){
	...
	for(j=0;j<(*mpop->pHF)[highestActiveLevel]->subpop_list.size();j++){//for each subpop of this level
		int subpop = (*mpop->pHF)[highestActiveLevel]->subpop_list[j];
		if (subpopID == subpop)
			return 1;
		else
			return 0;
