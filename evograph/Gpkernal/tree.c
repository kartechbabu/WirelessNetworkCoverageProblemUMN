...
/* modification for limiting the max numeric tree depth
 * MAX_NUMERIC_DEPTH
 *
 */
#define MAX_NUMERIC_DEPTH 4;//HJJ PARAM
				...
				)
{
     ...
     if ( depth == 0 )
     {
          ...
          i = random_int(fset->terminal_count_by_type[return_type])+
...
//		  printf("i = %d\n", i);
	 ...
	 if ( g->generateNodeType == 0) {       /// initial time for individual pop generation
		  /// we've reached root node, so choose a special list function
    	g->generateNodeType = 1;
			i = 0; 	// f_Tree function
	 }
	 else 
          ...
          switch ( (fset->cset_by_type[return_type])[i].type )
          {
             ...
             case EVAL_DATA:
               for ( j = 0; j < (fset->cset_by_type[return_type])[i].arity; ++j )
               {
				   ...
				   if((fset->cset_by_type[return_type])[i].argument_type[j]==OP_REAL &&
					   return_type !=OP_REAL) //for the parent node is real, then don't change the depth
					   depth = MAX_NUMERIC_DEPTH;//HJJ PARAM
				...
				)
{
	globaldata* g = get_globaldata();//get the lists
	 ...
	 else 	  // note that here we can generate a terminal node. 
     ...
     if ( lr )
     {
          ...
          for ( i = 0; i < count; ++i )
               if ( *lp == lr[i] )
               {
                     ...
                     */
                    NewNode = lw[i];
                    copy_tree_replace_many_recurse ( space, &NewNode, NULL,
                                                    NULL, count, repcount );
